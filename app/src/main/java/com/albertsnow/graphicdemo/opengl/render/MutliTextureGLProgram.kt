...
class MutliTextureGLProgram : AbsOpenGLProgram() {
    ...
    private var textureFirstId = -1
    private var textureSecondId: Int = 0
    ...
    override fun initProgram(context: Context) {
        ...
        GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, texture_buffer.limit() * 4,
                texture_buffer, GLES20.GL_DYNAMIC_DRAW)
        ...
        textureFirstId = TextureHelper.loadTexture(context, R.drawable.flower)
        textureSecondId = TextureHelper.loadTexture(context, R.drawable.fix_motor)
    }


    override fun draw(projectionMatrix: Matrix44F, cameraview: Matrix44F, modelMatrix: Matrix44F,
                      size: Vec2F) {
    ...
    private fun handleTexture() {
        if (countDown > 20) {
            ...
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureSecondId)
        } else {
            GLES20.glActiveTexture(GLES20.GL_TEXTURE0)
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureFirstId)
        }
